...
class WiLoR:
    ...
    @staticmethod
    def from_pretrained(weights_path, mano_path):
        """Load a WiLoR model from pre-converted MLX weights.

        No PyTorch dependency required. MANO model data must be provided
        separately due to its non-redistributable license.

        Args:
            weights_path: path to .safetensors file (ViT + RefineNet weights)
            mano_path: path to mano.npz (generated by convert CLI) or MANO_RIGHT.pkl
        Returns:
