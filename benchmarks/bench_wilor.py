...
def bench_mlx(weights_path, mano_path, warmup=30, iterations=100):
...
def main():
    ...
    parser.add_argument("--weights", default="weights/wilor-mlx.safetensors",
                        help="Path to wilor-mlx.safetensors")
    parser.add_argument("--mano-npz", default="weights/mano.npz",
                        help="Path to mano.npz (generated by convert CLI)")
    parser.add_argument("--ckpt", default=None,
                        help="Path to wilor_final.ckpt (pytorch backend)")
    parser.add_argument("--mano-pkl", default=None,
    ...
    if args.backend in ("mlx", "both"):
        ...
        mlx_result = bench_mlx(args.weights, args.mano_npz, args.warmup, args.iterations)
        results["benchmarks"].append(mlx_result)

    if args.backend in ("pytorch", "both"):
        if not all([args.ckpt, args.mano_pkl, args.mean_params]):
            print("ERROR: --ckpt, --mano-pkl, --mean-params required for pytorch backend")
            return
